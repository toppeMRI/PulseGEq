...
if 2*npair ~= length(varargs), pulsegeq.utils.fail('need names and values in pairs'), end
args = {varargs{1:2:end}};
vals = {varargs{2:2:end}};

if ~isempty(subs)
	if size(subs,2) ~= 2, pulsegeq.utils.fail('subs must be Nx2'), end
...
for ii=1:npair
	...
	if ~ischar(arg)
		pulsegeq.utils.fail('unknown option of class %s', class(arg))
	end
...
if isempty(idot)
	...
	elseif allow_extra
		...
		pulsegeq.utils.fail('unknown option name "%s"', arg)
	end
end
...
arg2 = arg([(idot+1):end]);
if ~isfield(opt, arg1)
	pulsegeq.utils.fail('unknown option1 name "%s"', arg1)
end
if ~isfield(opt.(arg1), arg2)
	pulsegeq.utils.fail('unknown option2 name %s', arg2)
end
s = struct('type', {'.', '.'}, 'subs', ...
...
try
	...
	printf(lasterr)
	pulsegeq.utils.fail('subsasgn? unknown option name %s', arg)
end
...
if ~isequal(opt.z, 'new'), pulsegeq.utils.fail('bug'), end
