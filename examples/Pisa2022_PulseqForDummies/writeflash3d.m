...
if arg.fatsat
    ...
    b1 = toppe.utils.rf.makeslr(fatsat.flip, fatsat.slThick, fatsat.tbw, fatsat.dur, 1e-6, sysGE, ...
        'type', 'ex', ...    % fatsat pulse is a 90 so is of type 'ex', not 'st' (small-tip)
        'writeModFile', false);
    b1 = toppe.makeGElength(b1);
    toppe.writemod(sysGE, 'rf', b1, 'ofname', 'fatsat.mod', 'desc', 'fat sat pulse');
end

% Excitation module
% Add zeros at end (non-zero nChop) for conversion to Siemens
toppe.utils.rf.makeslr(flip, arg.slabThick, ...
    arg.tbw, arg.rfDur, nz*arg.nCyclesSpoil, sysGE, ...
    'ftype', arg.ftype, ...
    'spoilDerate', 0.5, ...
    'ofname', 'tmp.mod');

[rf,gx,gy,gz,desc,paramsint16,paramsfloat,hdr] = toppe.readmod('tmp.mod');
rf = [rf; zeros(48,1)];
...
toppe.utils.makegre(FOV(1), N(1), voxSize(3), sysGE, ... 
...
toppe.write2loop('setup', sysGE, 'version', 4);  % initialize file ('scanloop.txt')
    ...
    for iy = 1:ny
        for ite = 1:length(DTE)
            ...
            if(arg.fatsat)
                ...
                fatFreq = arg.fatFreqSign*sysGE.gamma*1e4*sysGE.B0*fatChemShift*1e-6;  % Hz
                toppe.write2loop('fatsat.mod', sysGE, ...
            ...
            toppe.write2loop(arg.exMod, sysGE, ...
                'RFamplitude', 1.0, ...
                'textra', DTE(ite), ...
                'RFphase', rfphs);

            toppe.write2loop(arg.readoutMod, sysGE, ...
...
fprintf('\n');
toppe.write2loop('finish', sysGE);  % finalize file

fprintf('TR = %.3f ms\n', toppe.getTRtime(1, 2, sysGE)*1e3);

%figure; toppe.plotseq(1, 4, sysGE);

% Create 'sequence stamp' file for TOPPE
% This file is listed in line 6 of the .entry file
toppe.preflightcheck(arg.entryFile, 'seqstamp.txt', sysGE);

% Write files to tar archive (for convenience).
system(sprintf('tar cf flash3d.tar %s seqstamp.txt scanloop.txt modules.txt tipdown.mod readout.mod', arg.entryFile));

toppe.utils.scanmsg(arg.entryFile);

% Play sequence in loop (movie) mode
%nModulesPerTR = 2;
%toppe.playseq(nModulesPerTR, sysGE, ...
%    'tpause', 0.05, ...
%    'nTRskip', 8);
